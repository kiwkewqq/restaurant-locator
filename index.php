  ...
  <body>
      ...
      $(document).ready(function(){
        $('#btnSearch').click(function(){
          input = $('#txtSearch').val();
          var geocoder =  new google.maps.Geocoder();//set a new position
          geocoder.geocode( { 'address': input}, function(results, status) {
            ...
            } else if(input.length>0){ //check search box is not empty
              alert("Not found: "+input);
               point={lat:13.804294,lng:100.538179}; //default at Bang Sue
            }
            else{
               point={lat:13.804294,lng:100.538179}; //default at Bang Sue
            }
        ...
        $('#content').on('click','div[id^=restaurant]',function(){ //check card is clicked
        ...
        function initMap(){
          ...
          var request ={
              location:point,
              radius: 500, //distances from a location to restaurant
              types: ['restaurant']
          };
        ...
        function callback(results,status){ // create card of nearby restaurants in 500m
          if(status==google.maps.places.PlacesServiceStatus.OK){
            for (var i=0;i<results.length;i++){
              createMarker(results[i]['geometry']['location'],true,results[i]['name']);
        ...
        function clearMarkers(place,name) { //delete all markers on a map
        ...
        function clearMap(){ //create a new map with a new position
          setMapOnAll(null);
          markers=[];
          initMap();
        }
